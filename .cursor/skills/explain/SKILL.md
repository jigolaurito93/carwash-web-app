...
2. Classify the surface:
   ...
   - **Admin live** — `shop-info`, `categories`, `services`, `gallery`, `faq`
   - **Admin stub** — `dashboard`, `appointment`, `profile`
   - **Shared** — `lib/`, `proxy.ts`, `app/layout.tsx`, `app/api/`
3. Map data: which Supabase table, Server Action, or API route it uses. Live tables only: `categories`, `services`, `shop_info`, `shop_hours`, `site_announcements`, `gallery_images`, `faqs`. Call out leftover tables by name if present.
